...
function cleanAnswerText(value, sourceCount = 0) {
  ...
  text = repairNoisyChineseSublevelEnumeration(text);
  if (sourceCount > 0) {
...
function repairNoisyChineseSublevelEnumeration(value) {
  return String(value || "").replace(
    /((?:There are|There is|It has|The placement has|The course has|Chinese placement has)[^.!?\n]*?\b(?:nine|9)\s+(?:total\s+)?(?:(?:Chinese\s+)?placement\s+)?sub[- ]?levels?)(?::\s*[^.!?\n]*?\b[1-9][A-Z]\b[^.!?\n]*)?(\s*\[\d+\])?([.!?])/gi,
    (match, prefix, citation = "", punctuation = ".") => {
      const labels = Array.from(new Set(match.match(/\b[1-9][A-Z]\b/g) || []));
      const retainedCitation = citation || (match.match(/\s*\[\d+\](?=[.!?])/) || [""])[0];
      if (labels.length > 0 && labels.length < 9) {
        return `${prefix}${retainedCitation}${punctuation}`;
      }
      return match;
    }
  );
