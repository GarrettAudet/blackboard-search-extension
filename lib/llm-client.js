...
async function callChatCompletion({
  ...
  const jsonObjectMode = shouldRequestJsonObjectMode(messages);
  ...
  while (true) {
    ...
    if (finishReason && finishReason !== "stop") {
      ...
      if (safeReason === "error" && providerErrorRetries + 1 < CHAT_COMPLETION_TRANSIENT_ATTEMPTS) {
        providerErrorRetries += 1;
        await waitForProviderRetry(providerRetryBackoffMs(providerErrorRetries - 1));
        continue;
      }
...
async function requestChatCompletionJson({ config, model, messages, temperature, maxTokens, timeoutMs, jsonObjectMode = false }) {
  for (let attempt = 0; attempt < CHAT_COMPLETION_TRANSIENT_ATTEMPTS; attempt += 1) {
    ...
    try {
      response = await fetch(config.url, {
        ...
        body: JSON.stringify(chatCompletionRequestBody({ config, model, messages, temperature, maxTokens, jsonObjectMode }))
      });
...
function chatCompletionRequestBody({ config, model, messages, temperature, maxTokens, jsonObjectMode = false }) {
  const body = { model, messages, temperature, max_tokens: maxTokens };
  if (jsonObjectMode && config.supportsJsonObjectMode) body.response_format = { type: "json_object" };
  return body;
}

function shouldRequestJsonObjectMode(messages) {
  const text = (Array.isArray(messages) ? messages : [])
    .map((message) => String(message?.content || ""))
    .join("\n")
    .toLowerCase();
  return /return\s+(?:exactly\s+)?(?:one\s+)?json\s+object|return\s+json\s+only|response_format\s*:?\s*json|structured[-\s]?json/.test(text);
}
  ...
  return sources
    .map((source) => {
      ...
      return [
        ...
        source.document_coverage ? "document_coverage: " + field(source.document_coverage) : "",
        "document_coverage_complete: " + (source.document_coverage_complete === true ? "true" : "false"),
        "document_parent_scanned_complete: " + (source.document_parent_scanned_complete === true ? "true" : "false"),
