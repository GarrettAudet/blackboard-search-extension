...
function searchTermPosition(haystack, term) {
  ...
  if (/^\d+$/.test(value)) {
    const ordinal = new RegExp(`(?:^|\\s)${value}(?:st|nd|rd|th)(?=\\s|$)`).exec(text);
    if (ordinal) return ordinal.index;
  }
...
function prepareAnswerSources(results, query = "") {
  ...
  const routedResults = isPackingQuery(query)
    ? prioritizeDomainResults(results || [], isPackingResult, query)
    : isVisaQuery(query)
      ? prioritizeDomainResults(results || [], isVisaResult, query)
      : isTaskDeadlineQuery(query)
        ? prioritizeTaskDeadlineResults(results || [])
        : isCourseListQuery(query)
          ? prioritizeCourseListResults(results || [])
          : results || [];
  let searchResults = dedupeSourceCandidates(rankSourceCandidates(routedResults, query), query);
  if (/\b(?:official|blackboard)\b/.test(normalizeText(query))) {
    if (isPackingQuery(query)) searchResults = prioritizeDomainResults(searchResults, isPackingResult, query);
    else if (isVisaQuery(query)) searchResults = prioritizeDomainResults(searchResults, isVisaResult, query);
  }
  const selected = [];
  const seen = new Set();
...
function enhanceRetrievalQueryForIntent(query, retrievalQuery = query, plan = null) {
  ...
  if (isRestrictedIndependentTravelPermitQuery(query)) {
    pieces.push("Tibet Travel Permit foreigners licensed tour operator cannot travel independently rules change confirm current requirements");
  }
...
function isRestrictedIndependentTravelPermitQuery(query) {
  const normalized = normalizeText(query);
  return (
    /\bpermit\b/.test(normalized) &&
    /\bindependent(?:ly)?\b/.test(normalized) &&
    /\btravel\b/.test(normalized) &&
    /\b(?:restriction|rules?|current|foreigners?|operator|licensed|verify|recheck|confirm)\b/.test(normalized)
  );
}
...
function isVisaQuery(query) {
  const normalized = normalizeText(query);
  if (/\b(?:x1|visa|jw202|jw201|residen(?:ce|t)\s+permit|passport|embassy|consulate|admission\s+notice|immigration)\b/.test(normalized)) {
    return true;
  }
  return (
    /\bpermit\b/.test(normalized) &&
    /\b(?:residence|entry|student|public security|after entering|within 30 days|register|registration)\b/.test(normalized)
  );
}
...
function prioritizeDomainResults(results, predicate, query = "") {
  const domainResults = (results || []).filter(predicate);
  if (!domainResults.length) return results || [];
  if (/\b(?:official|blackboard)\b/.test(normalizeText(query))) {
    domainResults.sort((a, b) => {
      const officialA = searchResourceSourceClass(a) === "official_blackboard" && searchResourceHasValidatedAuthority(a);
      const officialB = searchResourceSourceClass(b) === "official_blackboard" && searchResourceHasValidatedAuthority(b);
      return Number(officialB) - Number(officialA);
    });
  }
...
function isCourseListQuery(query) {
  ...
  if (
    /\b(?:room|rooms|classroom|classrooms)\b/.test(normalized) &&
    /\b(?:assign|assigned|assignment|assignments|number|numbers|meet|meets|held|located|location|venue)\b/.test(normalized) &&
    /\b(?:course|courses|class|classes|schedule|calendar)\b/.test(normalized)
  ) {
    return true;
  }
  if (/\bwhere\b.{0,50}\b(?:course|courses|class|classes)\b.{0,50}\b(?:meet|meets|held|located|scheduled)\b/.test(normalized)) {
    return true;
  }
...
function sourceQualityScore(result, query = "") {
  ...
  score += sourceQueryMatchQualityScore(result, query);
...
function sourceQueryMatchQualityScore(result, query = "") {
  const profile = searchQueryProfile(query);
  const terms = profile.baseTokens || [];
  ...
  let score = queryCoverageBoost(profile, title, text, source);

  // A short passage that covers most of a specific question is stronger evidence
  // than a long parent document that happens to repeat a few generic terms.
  if (text && text.length <= 2000 && coverage >= 0.7) score += 120;

  const numericTerms = terms.filter((term) => /^\d+$/.test(term));
  if (numericTerms.length && numericTerms.every((term) => searchTermPosition(`${title} ${text}`, term) >= 0)) {
    score += 100;
  }
...
function mergeSourceCandidateGroup(group, query = "") {
  ...
  const corePassages = selectDiverseSourcePassages(ranked, query, 6, 9000);
  const passageCandidates = appendSourceCandidateNeighbors(corePassages, query, 8, 14000);
  const excerpts = [];
  ...
  for (const candidate of passageCandidates) {
    ...
    if (excerpts.length >= 8 || (totalChars && totalChars + rawText.length > 14000)) continue;
...
function expandedTokens(query) {
  ...
  const synonymMap = {
    ...
    visa: ["x1", "jw202", "permit", "residence", "residence permit", "resident permit"],
    vpn: ["reliability", "redundancy", "backup", "fallback", "obfuscated", "esim", "moving target", "current cohort"],
    permit: ["permission", "authorization", "license", "residence permit", "resident permit", "x1", "jw202"],
    ...
    bedroom: ["bedrooms", "room", "rooms", "single room", "private bathroom"],
    bedrooms: ["bedroom", "room", "rooms", "single room", "private bathroom"],
    room: ["rooms", "bedroom", "bedrooms"],
    rooms: ["room", "bedroom", "bedrooms"],
    setup: ["arrangement", "configuration", "room", "provided"],
    tour: ["tours", "virtual tour", "online tour", "website"],
    tours: ["tour", "virtual tour", "online tour", "website"],
    online: ["virtual", "website", "web"],
    location: ["where", "website", "address"],
    equipment: ["hardware", "device", "devices", "laptop", "smartphone"],
    funding: ["funded", "covers", "covered", "included", "stipend"],
    funded: ["funding", "covers", "covered", "included", "stipend"],
    partner: ["partners", "spouse", "spouses"],
    partners: ["partner", "spouse", "spouses"],
    ...
    mandarin: ["chinese", "language", "language study", "course", "study", "level", "proficiency", "intermediate", "advanced"],
    chinese: ["mandarin", "language", "language study", "course", "level", "proficiency"],
    language: ["mandarin", "chinese", "language study", "course", "level", "proficiency", "intermediate", "advanced"],
    coordinator: ["coordinate", "organize", "organizer", "volunteer", "position"],
    mentor: ["mentors", "mentorship", "match", "matched", "wider pool", "permission", "relationships"],
    mentors: ["mentor", "mentorship", "match", "matched", "wider pool", "permission", "relationships"],
    assigned: ["match", "matched"],
    credit: ["credits", "degree", "requirement", "complete"],
    minimum: ["at least", "required", "requirement"],
    mainland: ["China", "Chinese", "students"],
    international: ["foreign", "students", "degree"],
  ...
  if (
    normalizedQuery.includes("permit") &&
    normalizedQuery.includes("independent") &&
    normalizedQuery.includes("travel")
  ) {
    extras.push("Tibet", "Tibet Travel Permit", "foreigners", "licensed tour operator", "cannot travel independently", "rules change");
  }
