...
function searchIndex(query, limit = 10) {
  ...
  const scored = candidateSearchDocs(query, queryProfile)
    .map((doc) => ({ ...doc, score: scoreDoc(query, doc, queryProfile) }))
    .filter((doc) => doc.score > 0)
    .sort(compareSearchResultsByScore);
...
function candidateSearchDocs(query, queryProfile) {
  ...
  if (!postingLists.length) return corpus.docs;
  ...
  if (!selected.size) return corpus.docs;
...
function searchPostingList(corpus, token) {
  ...
  for (const indices of groups) {
    ...
    if (
      ...
      // Identical search documents score identically, so one representative is enough before source deduplication.
...
function cachedSearchCorpus(query) {
  const mode = wantsVideoHeavySearch(query) ? "video" : "standard";
  ...
  docs.forEach((doc, index) => {
    const signature = `${doc.search_title}\u001f${doc.search_source}\u001f${doc.search_text}`;
  ...
  const corpus = {
    ...
    docs,
    postings: new Map(),
    posting_groups: Array.from(groupsBySignature.values()).map((group) =>
      [...group].sort((a, b) => compareSearchResultIdentity(docs[a], docs[b]))
    )
  };
...
function sourceQualityScore(result, query = "") {
  ...
  } else if (provenance === "program webinar transcript") {
...
function selectDiverseSourcePassages(ranked, query = "", limit = 3, totalCharLimit = 5600) {
  ...
  const profiles = ranked.map((candidate, rankIndex) => {
    ...
    return { candidate, rankIndex, text, termSet, routeRanks, contentTokens };
  });
...
function canonicalDocumentKey(result) {
  if (result?.source_pack_id && result?.source_pack_document_id) return `packdoc:${result.source_pack_id}:${result.source_pack_document_id}`;
  if (result?.source_pack_id && result?.resource_id) return `packdoc:${result.source_pack_id}:${result.resource_id}`;
  if (String(result?.kind || "").toLowerCase() === "page") {
    ...
    if (pageTitle && pageSource) return `page:${pageTitle}|${pageSource}`;
  }
...
function buildSearchDocs(query = "") {
  ...
  for (const resource of state.resources) {
    ...
    if (!processed) {
      const storedContent = cleanIndexedText(rawStoredContent);
      if (shouldSkipResourceSearchDoc(resource, storedContent, wantsVideo)) {
        ...
        processed = normalizeText(fullText)
          ? {
              skip: false,
              resourceTitle,
              sourceTrail,
              hasBody,
              texts: fullText.length > 1200 ? chunkTextForSearch(fullText, 1400, true) : [fullText]
            }
          : { skip: true };
      }
